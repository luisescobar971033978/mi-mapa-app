...
export const inicializarAgenda = async (client, tableId, fechaInputId, horaHiddenId, btnSubmitId) => {
    ...
    // --- 3. GESTIÓN DE MODAL Y FLUJO AUTOMÁTICO DE SOLICITUD ---
    if (btnSubmit) {
        btnSubmit.addEventListener('click', async (e) => {
            e.preventDefault(); // Frenamos el comportamiento por defecto para controlar la secuencia

            const fechaSel = fechaInput.value;
            const horaSel = horaHidden.value;

            if (!fechaSel || !horaSel) {
                alert("Por favor seleccione una fecha y hora en la agenda.");
                return;
            }

            const notifAceptada = localStorage.getItem('notificacion_aceptada');

            // Función que realiza el guardado y redirige al mapa
            const procesarYRedirigir = async () => {
                btnSubmit.innerText = "SOLICITANDO...";
                btnSubmit.disabled = true;
                ...
                try {
                    ...
                    if (publicVapidKey && publicVapidKey !== 'TU_CLAVE_PUBLICA_VAPID_AQUI') {
                        try {
                            ...
                            console.warn("No se pudo registrar Push (posible modo incógnito):", pushErr);
                        }
                    }

                    // Recuperamos el ID de la solicitud actual
                    ...
                    // REDIRECCIÓN INMEDIATA A MAPA.HTML
                    console.log("¡Cita agendada con éxito! Redirigiendo a mapa.html...");
                    window.location.href = 'mapa.html';

                } catch (err) {
                    console.error("Error en el proceso de solicitud:", err);
                    btnSubmit.innerText = "SOLICITAR SERVICIO";
                    btnSubmit.disabled = false;
                }
            };

            // Si ya aceptó antes las notificaciones, ejecutamos el proceso directo sin mostrar el modal
            if (notifAceptada || (("Notification" in window && Notification.permission === "granted"))) {
                await procesarYRedirigir();
                return;
            }

            // Si es la primera vez, mostramos el modal amigable de advertencia
            ...
            // Al hacer clic en aceptar del modal, pedimos permiso y disparamos el flujo automático hacia mapa.html
